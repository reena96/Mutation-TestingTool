...
public class Test_2 {
	...
	static class ViolentStringMatcher implements StringMatcher {
		...
		public int indexOf(String source, String pattern) {
			...
			while (i < sLen && j < pLen) {
				Template.instrum("44", "WhileStatement",
						"i < sLen && j < pLen", "pLen", pLen, "sLen", sLen,
						"i", i, "j", j);
				if (src[i] == ptn[j]) {
					Template.instrum("45", "IfStatement", "src[i] == ptn[j]",
							"src", src, "i", i, "j", j, "ptn", ptn);
					// 如果当前字符匹配成功,则将两者各自增1,继续比较后面的字符
					i++;
					Template.instrum("47", "PostfixExpression", "i++", "i", i);
					j++;
					Template.instrum("48", "PostfixExpression", "j++", "j", j);
				} else {
					// 如果当前字符匹配不成功,则i回溯到此次匹配最开始的位置+1处,也就是i = i - j + 1
					// (因为i,j是同步增长的), j = 0;
					i = i - j + 1;
					Template.instrum("52", "Assignment", "i=i - j + 1", "i", i,
							"j", j);
					j = 0;
					Template.instrum("53", "Assignment", "j=0", "j", j);
				}
			}
	...
	static class KMPStringMatcher implements StringMatcher {
		...
		protected int[] getNext(char[] p) {
			...
			Template.instrum("89", "Assignment", "next[0]=-1", "next", next);
			while (j < pLen - 1) {
				Template.instrum("90", "WhileStatement", "j < pLen - 1",
						"pLen", pLen, "j", j);
				if (k == -1 || p[j] == p[k]) {
					Template.instrum("91", "IfStatement",
							"k == -1 || p[j] == p[k]", "p", p, "j", j, "k", k);
					k++;
					Template.instrum("92", "PostfixExpression", "k++", "k", k);
					j++;
					Template.instrum("93", "PostfixExpression", "j++", "j", j);
					next[j] = k;
					Template.instrum("94", "Assignment", "next[j]=k", "next",
							next, "j", j, "k", k);
				} else {
					k = next[k];
					Template.instrum("96", "Assignment", "k=next[k]", "next",
							next, "k", k);
				}
			}
			return next;
		...
		public int indexOf(String source, String pattern) {
			...
			while (i < sLen && j < pLen) {
				Template.instrum("110", "WhileStatement",
						"i < sLen && j < pLen", "pLen", pLen, "sLen", sLen,
						"i", i, "j", j);
				// 如果j = -1,或者当前字符匹配成功(src[i] = ptn[j]),都让i++,j++
				if (j == -1 || src[i] == ptn[j]) {
					Template.instrum("112", "IfStatement",
							"j == -1 || src[i] == ptn[j]", "src", src, "i", i,
							"j", j, "ptn", ptn);
					i++;
					Template.instrum("113", "PostfixExpression", "i++", "i", i);
					j++;
					Template.instrum("114", "PostfixExpression", "j++", "j", j);
				} else {
					// 如果j!=-1且当前字符匹配失败,则令i不变,j=next[j],即让pattern模式串右移j-next[j]个单位
					j = next[j];
					Template.instrum("117", "Assignment", "j=next[j]", "next",
							next, "j", j);
				}
			}
			if (j == pLen)
	...
	static class OptimizedKMPStringMatcher extends KMPStringMatcher {
		...
		protected int[] getNext(char[] p) {
			...
			int j = 0;
			next[0] = -1; // next数组中next[0]为-1
			Template.instrum("145", "Assignment", "next[0]=-1", "next", next);
			while (j < pLen - 1) {
				Template.instrum("146", "WhileStatement", "j < pLen - 1",
						"pLen", pLen, "j", j);
				if (k == -1 || p[j] == p[k]) {
					Template.instrum("147", "IfStatement",
							"k == -1 || p[j] == p[k]", "p", p, "j", j, "k", k);
					k++;
					Template.instrum("148", "PostfixExpression", "k++", "k", k);
					j++;
					Template.instrum("149", "PostfixExpression", "j++", "j", j);
					// 修改next数组求法
					if (p[j] != p[k]) {
						Template.instrum("151", "IfStatement", "p[j] != p[k]",
								"p", p, "j", j, "k", k);
						next[j] = k;// KMPStringMatcher中只有这一行
						Template.instrum("152", "Assignment", "next[j]=k",
								"next", next, "j", j, "k", k);
					} else {
						// 不能出现p[j] = p[next[j]],所以如果出现这种情况则继续递归,如 k = next[k],
						// k = next[[next[k]]
						next[j] = next[k];
						Template.instrum("156", "Assignment",
								"next[j]=next[k]", "next", next, "j", j, "k", k);
					}
				} else {
					k = next[k];
					Template.instrum("159", "Assignment", "k=next[k]", "next",
							next, "k", k);
				}
			}
			return next;
